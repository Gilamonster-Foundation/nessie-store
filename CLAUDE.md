...
- **Current state (as of 0.3.1) — plan vs. reality.** The README
  ...
  mint-once identity) over the trait stack; the `zfs` + `mem` backends; an embedded
  hardened NFSv3 *data* plane (`nessie-nfs` + `nessie-nfsserve`); and SnapMirror's live
  data plane (`ReplicationBackend`, real `zfs send`/`zfs receive` bytes — see
  [docs/status.md](docs/status.md)). **Not yet done:** the live-ZFS / Trident acceptance
  gate is unbuilt (#70); the s3 / nfs-bare / ontap-passthrough / p4d / gitlfs backends and the unified
